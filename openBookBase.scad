...
l = 340;
w = 220;
//Looking into new panel mount stuff now. should get thinner.
h = 22;
...
HEATSET_INSERT_HOLE = 2.1;

//translate([0,0,-8]) import("topPlate.stl");
...
difference() {
	...
	minkowski(){
    	...
    	//Side intake. ~20mm fan would be hard to find and ineffecient. I want one (or three).
    	for(a = [0 : 1 ]) {
            for(i = [0 : 13]) {
    ...
    //Dc-Dc buck
    //translate([-51, 40, -20]) m3Hole();
	//translate([-20, 27, -20]) m3Hole();
	...
	//Mounting holes for 2s 18650 holders
	for(i = [0 : 1]) {
	...
	//Circular  cutouts for switches & I/O
	rotate([0, 90, 0]) {
    		//Key
	...
	translate([0, w/2-38, -10]) {
        cube([psuHoleL, psuHoleW, 10], center=true);
        //translate([psuHoleL/2-7, 0, -10])
        //cube([2, 99, 40]);
    }
